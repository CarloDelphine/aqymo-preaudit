...
Réponds UNIQUEMENT en JSON strict, champs courts (1-2 phrases max) :
{
  ...
  "donnees_officielles": {
    "transactions_dvf": "Estimation experte utilisée — pas de DVF API",
...
async function lancerAnalyseBackground(missionId, data) {
  ...
  const promptText = buildPrompt(adresse, prix, surface, type_bien, annee, dpe_classe, notes_client, prixM2, lat, lon, label, codeInsee, govDataSummary, []);
    ...
    const req = https.request(options, (apiRes) => {
      ...
      apiRes.on('end', async () => {
        try {
          ...
          result._missionData = data;
          result._status = 'ready';
          await redisSet(`mission:${missionId}`, result);
        } catch (e) {
          await redisSet(`mission:${missionId}`, { _status: 'error', error: e.message });
        }
        resolve();
      });
    });
...
module.exports = async function handler(req, res) {
  ...
  const { adresse, prix, surface, type_bien, annee, dpe_classe, notes_client, email_client, projet_client } = req.body;
  if (!adresse) return res.status(400).json({ error: 'Adresse manquante' });

  // Génère un ID de mission unique
  const missionId = Date.now().toString(36) + Math.random().toString(36).slice(2, 6);

  // Stocke les données client immédiatement
  const missionData = { adresse, prix, surface, type_bien, annee, dpe_classe, notes_client, email_client, projet_client, missionId, createdAt: new Date().toISOString(), _status: 'pending' };
  await redisSet(`mission:${missionId}`, missionData);

  // Retourne confirmation immédiate
  res.status(200).json({ ok: true, missionId });

  // Lance l'analyse en arrière-plan (après la réponse)
  lancerAnalyseBackground(missionId, missionData).catch(console.error);
};
